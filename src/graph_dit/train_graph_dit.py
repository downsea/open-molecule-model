...
from src.graph_dit.config import UnifiedConfigManager
...
def main():
    ...
    parser.add_argument('--data-path', type=str, default='data/standard',
    ...
    parser.add_argument('--save-dir', type=str, default='checkpoints/graphdit',
                        help='Directory to save checkpoints')
    parser.add_argument('--log-dir', type=str, default='logs/graphdit',
    ...
    train_info = {'num_graphs': len(loaders['train'].dataset)} if hasattr(loaders['train'], 'dataset') else {}
    val_info = {'num_graphs': len(loaders['val'].dataset)} if hasattr(loaders['val'], 'dataset') else {}
    test_info = {'num_graphs': len(loaders['test'].dataset)} if hasattr(loaders['test'], 'dataset') else {}
    ...
    trainer = GraphDiTTrainer(
        ...
        scheduler=torch.optim.lr_scheduler.CosineAnnealingLR(
            torch.optim.AdamW(model.parameters(), lr=float(config.training.learning_rate)),
